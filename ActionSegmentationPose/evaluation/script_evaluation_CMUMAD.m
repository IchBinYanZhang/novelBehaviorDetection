        ...
        for ss = subject
            ...
            for qq = 1:2
                ...
                if strcmp(pose_feature, 'jointLocs')
                    ...
                    for ii = 1:size(pattern,2)/3
                        pattern(:,3*ii-2) = pattern(:,3*ii-2)-pattern(:,1);
                        pattern(:,3*ii-1) = pattern(:,3*ii-1)-pattern(:,2);
                        pattern(:,3*ii) = pattern(:,3*ii)-pattern(:,3);
                    end
                    pattern(:,1:3) = []; 
...
%                     disp('--postprocessing, merge clusters');
                    idx = calLocalFeatureAggregationAndClustering(pattern,idx1,C, 36,'ours'); 

                end
                CptTime = [CptTime toc(startTime)];
                res = funEvalDetection_CMUMAD(gtlabE, double(idx'), 0.5, false); % tol range = 15
%                 yt = zeros(size(idx));
