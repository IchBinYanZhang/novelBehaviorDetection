%%% (1) script to evaluate the abnormality recognition in BOMNI
...
%     for vv = 3:3
        ...
        if strcmp(method, 'spectralClustering')
        ...
        elseif strcmp(method,'ours')
            ...
            %%%% uncomment one of the following to enable a temporal
            %%%% pooling method.
            idx = fun_feature_aggregation(pattern,idx1,c_locs,'ours','BOMNI'); %% motion energy-based 
%             idx = fun_feature_aggregation_slidingwindow(pattern,c_locs,2.5e-2); %% sliding window-based
%             idx = fun_feature_aggregation_kernelizedcut(pattern,c_locs,1e-3,'sequential'); %% kernelised cut-based
