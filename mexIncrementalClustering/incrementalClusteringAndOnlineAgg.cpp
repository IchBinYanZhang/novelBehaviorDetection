...
void temporalSpline(cv::Mat& src, cv::Mat& dst){
    ...
    static const float alpha = global.temporal_reg_alpha;
    ...
    for (int i = 0; i < n_dims; i++){
        ...
        for(int it = 0; it < iterMax; it++){
            for(int t = bt; t < bt+n_samples; t++){
                ...
                float phi_prime = 1.0f/sqrtf( std::pow(0.5*u.at<float>(t+1)-0.5*u.at<float>(t-1), 2) + eps);
...
// Work: (1) temporal interpolation and (2) keyframe selection
{
    ...
    for (int i = 0; i < n_frames; i++) {
        global.output_vec_KFramePool.push_back(joints.row(i));
...
void runIncrementalClustering() {
    ...
    jointsTemporalSmoothingKeyframeSelection(); // tv-l1 temporal regularity. TVL1 is performed for each individual channel
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs != 6)
        mexErrMsgIdAndTxt("Three arguments should be parsed", 
            "usage: fun(samples, ShortMemorySize, errorEstimate, USE_TEMPORAL_REG, VERBOSE, TVL1_weights)");
